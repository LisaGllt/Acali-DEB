...
file_path <- here::here("mod/DEB_SAAWK")
...
seeds <- c(C1 = 3333, C2 = 6665, C3 = 1214)
...
# ./mcsim.DEBcaliAWK DEB_sim_full.in
...
df_sim_long <- df_sim %>%
  ...
  filter(!(Variable %in% c("OM_out", "Qf", "Qfmax", "X"))) |> 
  mutate(Variable = fct_relevel(Variable, 
                                "Energy", "Size", "Weight", "Reproduction", "Maturity", "Organic_matter", "Length_struct",
                                "freal", "kapreal"))
...
for (exp_id in experiences) {
  ...
  mutate(
    Variable = factor(
      Variable,
      levels = c("Energy", "Length_struct", "Weight", "Reproduction", "Maturity", "Organic_matter",
                 "freal", "kapreal")
    )
  )
